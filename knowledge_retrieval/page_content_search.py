...
from knowledge_base.database_sync import TextPageRepositorySync as TextPageRepository, ChunkPageRepositorySync as ChunkPageRepository
...
class PageContentSearch:
    ...
    async def get_multiple_pages(
        ...
        """
        try:
            target_pages = TextPageRepository.get_pages_by_indices(
                file_id=file_id,
                page_indices=page_indices
            )

            if not target_pages:
                logger.warning(f"文件 {file_id} 中没有找到指定的页面数据")
                return {
                    "error": f"file_id: {file_id}。该文件中没有找到指定的页面数据"
                }
    ...
    async def get_page_chunks_info(
        self,
        file_id: int,
        page_index: int
    ) -> List[Dict[str, Any]]:
        """
        获取指定页面下的所有chunk信息。file_id: 文件ID。page_index: 页面索引
        返回chunk信息列表，包含chunk_id, chunk_index, file_id等
        """
        try:
            chunks_info = ChunkPageRepository.get_chunks_by_file_and_page_index(
                file_id=file_id,
                page_index=page_index
            )
            return chunks_info
        except Exception as e:
            logger.error(f"获取页面chunk信息失败: {e}")
            return []
