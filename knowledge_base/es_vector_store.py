...
class ESVectorStore:
    ...
    def create_index(self, shards: int = 1, replicas: int = 0, overwrite: bool = True) -> bool:
        """
        创建索引，定义 mappings
        """
    ...
    def bulk_index(self, documents: List[Dict[str, Any]], chunk_size: int = 100) -> tuple:
        ...
        success, failed = helpers.bulk(
            self.es, actions, chunk_size=chunk_size, stats_only=True, raise_on_error=False, refresh=True
        )
    ...
    def search_vector(
        ...
        Args:
            query_vector: 查询向量，不传则自动对query_text向量化
            query_text: 查询文本，不传则使用query_vector
            k: 返回的最相似文档数量
            num_candidates: 每个分片考虑的候选文档数
            source_fields: 需要返回的字段列表

        Returns:
            文档列表，每项包含 "_id", "_score", "_source"
        """
    ...
    def search_text_fuzzy(
        ...
        query = {
            "query": {
                "match": {self.text_field: {"query": query_text, "fuzziness": fuzziness}}
            },
            "size": size,
        }
        if source_fields:
